...
export function validate<TValue>(
  ...
  if (isFormState<TValue>(stateOrFunctionOrFunctionArray)) {
    ...
    const optionalRemoved = formStateReducer<TValue>(
      stateOrFunctionOrFunctionArray,
      new SetUserDefinedPropertyAction(state.id, 'optional', undefined),
    );

    return formStateReducer<TValue>(optionalRemoved, new SetErrorsAction(state.id, errors));
  }
  ...
  let updateFnArr = Array.isArray(functionOrFunctionArray)
    ...
    : [functionOrFunctionArray];
  updateFnArr =
    functionOrFunctionArr === undefined ? updateFnArr : updateFnArr.concat(functionOrFunctionArr);
